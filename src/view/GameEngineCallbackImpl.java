...
public class GameEngineCallbackImpl implements GameEngineCallback
{
   ...
   public void nextCard(Player player, PlayingCard card, GameEngine engine)
   {
      ...
      logger.log(Level.FINE,String.format(
    		  "Card Dealt to %s .. %s", player.getPlayerName(), card.toString()));
   }
   ...
   public void result(Player player, int result, GameEngine engine)
   {
      ...
      logger.log(Level.INFO, String.format(
    		  "%s, final result=%s", player.getPlayerName(), player.getResult()));
   }

	@Override
	public void bustCard(Player player, PlayingCard card, GameEngine engine) {
		logger.log(Level.FINE,String.format(
	    		  "Card Dealt to %s .. %s ... YOU BUSTED!", player.getPlayerName(), card.toString()));
	}
	
	@Override
	public void nextHouseCard(PlayingCard card, GameEngine engine) {
		logger.log(Level.FINE,String.format(
	    		  "Card Dealt to House .. %s", card.toString()));	
	}
	
	@Override
	public void houseBustCard(PlayingCard card, GameEngine engine) {
		// INFO: Card Dealt to House .. Suit: Diamonds, Value: Ten, Score: 10 ... HOUSE BUSTED!
		logger.log(Level.FINE,String.format(
	    		  "Card Dealt to House .. %s ... HOUSE BUSTED!", card.toString()));
	}
	
	@Override
	public void houseResult(int result, GameEngine engine) {
		logger.log(Level.INFO, String.format(
	    		  "House, final result=%s", result));
		
		for(Player eachPlayer : engine.getAllPlayers()) {
			logger.log(Level.INFO, eachPlayer.toString());
		}
	}
}
